...
export function registerFileOps(server) {
    ...
    server.tool('save_file', 'Write a code file to the workspace. Used by agents to create actual project files. Records the file in the context board for tracking.', {
        ...
        if (!resolved.startsWith(path.resolve(workspacePath))) {
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'Path traversal not allowed.' }) }],
            };
        }
    ...
    // --- track_file ---
    server.tool('track_file', 'Record a file in the context board without writing it. Use this when the file was already written by Gemini\'s native file tools — this just updates the Weaver tracking.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    // --- read_file ---
    server.tool('read_file', 'Read the contents of a file from the workspace. Returns file content and metadata. Use this to inspect existing code before making changes.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        filePath: z.string().describe('Relative file path from workspace root (e.g., "src/index.ts")'),
        ...
        if (!fs.existsSync(resolved)) {
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: `File not found: ${filePath}` }) }],
            };
        }
        try {
            const stat = fs.statSync(resolved);
            if (stat.isDirectory()) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ success: false, message: `Path is a directory, not a file: ${filePath}` }) }],
                };
            }
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: `Error reading file: ${err instanceof Error ? err.message : 'Unknown error'}` }) }],
            };
        }
    });
    // --- delete_file ---
    server.tool('delete_file', 'Delete a file from the workspace and remove it from tracking. Use this when a file is no longer needed (e.g., during refactoring).', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: `Error deleting file: ${err instanceof Error ? err.message : 'Unknown error'}` }) }],
            };
        }
    });
    // --- list_files ---
