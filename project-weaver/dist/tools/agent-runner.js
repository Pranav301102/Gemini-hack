...
export function registerAgentRunner(server) {
    ...
    // --- request_revision ---
    server.tool('request_revision', 'Request changes from the developer after a code review. Resets the developer agent to working status and records the feedback. Used by the Code Reviewer when issues are found.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        const revisionEntries = board.entries.filter(e => e.type === 'proposal' && e.metadata?.isRevisionRequest === true);
