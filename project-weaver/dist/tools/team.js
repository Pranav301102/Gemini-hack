...
// ─── Git helpers ───
function getGitUser(workspacePath) {
    try {
...
function getGitContributors(workspacePath) {
    try {
...
function getWeaverFileStatus(workspacePath) {
    try {
...
// ─── File I/O helpers ───
function readTeamState(workspacePath) {
...
function writeTeamState(workspacePath, state) {
    const filePath = path.join(workspacePath, '.weaver', TEAM_FILE);
    fs.writeFileSync(filePath, JSON.stringify(state, null, 2), 'utf-8');
}
function readAnnotations(workspacePath) {
...
function writeAnnotations(workspacePath, store) {
    const filePath = path.join(workspacePath, '.weaver', ANNOTATIONS_FILE);
    fs.writeFileSync(filePath, JSON.stringify(store, null, 2), 'utf-8');
}
// ─── Registration ───
export function registerTeam(server) {
    // --- team_status ---
    server.tool('team_status', 'Get the team collaboration status: git contributors, who has scanned the project, shared .weaver state, annotation counts, and task claims. Use this to understand the team landscape before making changes.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
    }, async ({ workspacePath }) => {
        const manager = new BoardManager(workspacePath);
        if (!manager.exists()) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'Project not initialized. Run init_project first.' }) }] };
        }
    ...
    server.tool('record_team_activity', 'Record a team member activity (scan, build, implement, review, annotate) for collaboration tracking. Called automatically by other tools.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (member) {
            member.lastActive = new Date().toISOString();
            if (action === 'scan')
                member.contributions.scans++;
            if (action === 'annotate')
                member.contributions.annotations++;
            if (action === 'review')
                member.contributions.reviews++;
            if (action === 'build')
                member.contributions.builds++;
        }
        ...
        writeTeamState(workspacePath, team);
        return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, message: `Recorded ${action} by ${memberName}` }) }],
        };
    });
    // --- add_team_note ---
    server.tool('add_team_note', 'Add a shared team note visible to all team members and agents. Use for blockers, decisions, handoffs between team members.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, noteId: note.id, message: `Team note added by ${note.author}` }) }],
        };
    });
    // --- claim_task ---
    server.tool('claim_task', 'Claim a planned change so other team members know you are working on it. Prevents duplicate work.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (existing && existing.status !== 'done') {
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: `Already claimed by ${existing.claimedBy} (${existing.status})` }) }],
            };
        }
    ...
    // --- verify_annotation ---
    server.tool('verify_annotation', 'Mark an agent-written code annotation as human-verified. This confirms the annotation is accurate and trustworthy for the whole team.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (!annotation) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'Annotation not found' }) }] };
        }
        ...
        writeAnnotations(workspacePath, store);
        return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, message: `Annotation verified by ${annotation.verifiedBy}`, symbol: annotation.symbol }) }],
        };
    });
    // --- get_annotations ---
    server.tool('get_annotations', 'Get all code annotations, optionally filtered by file or tag. Returns detailed notes about how code works, written by agents and verified by humans.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (file)
            filtered = filtered.filter(a => a.file === file);
        if (tag)
