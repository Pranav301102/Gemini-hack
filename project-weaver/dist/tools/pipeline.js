...
import { PIPELINE_STAGES, STAGE_AGENT_MAP, STAGE_DESCRIPTIONS, AGENT_DISPLAY_NAMES, STAGE_NAMES, } from '../types.js';
...
export function registerPipeline(server) {
    server.tool('run_pipeline', 'Run the full AI Software Agency pipeline: read -> architecture -> spec -> stories -> approval -> implementation -> testing -> review -> ship. Returns a structured execution plan for Gemini to follow step by step.', {
        ...
        const plan = activeStages.map((stage, index) => {
            const agent = STAGE_AGENT_MAP[stage];
            return {
                step: index + 1,
                stage,
                agent,
                agentDisplayName: agent === 'user' ? 'User (Approval Gate)' : AGENT_DISPLAY_NAMES[agent],
...
${hasApproval ? '\n## ⚠️ APPROVAL GATE\nThe pipeline will PAUSE at the "approval" stage. The user must review and approve from the dashboard before implementation begins. Do NOT skip this step.' : ''}`,
    ...
    server.tool('advance_pipeline', 'Manually advance a pipeline stage to in-progress or complete.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
