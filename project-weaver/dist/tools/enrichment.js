 ...
 */
function extractCodeSnippet(rootPath, filePath, line, kind) {
 ...
 */
function collectUnenrichedItems(index, kindFilter, fileFilter, skipEnriched = true) {
    const items = [];
    let totalItems = 0;
    let enrichedItems = 0;
    for (const file of index.files) {
        if (fileFilter && !file.path.includes(fileFilter))
            continue;
        // Functions
...
export function registerEnrichment(server) {
    server.tool('enrich_index', 'Get a batch of un-enriched code items (functions, classes, types) with code snippets. Read each item and call save_enrichments with one-line descriptions. This is the core agent memory building step.', {
    ...
    server.tool('save_enrichments', 'Save LLM-generated descriptions back to the code index. Call after reading the batch from enrich_index.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        for (const enrichment of enrichments) {
            ...
            if (enrichment.kind === 'function') {
                ...
                if (cls) {
                    ...
                    if (enrichment.purpose)
                        cls.purpose = enrichment.purpose;
                    updated++;
                }
            }
