...
export function registerContextBoardTools(server) {
    // --- get_context_board ---
    server.tool('get_context_board', 'Read the shared context board showing all agent decisions, artifacts, and project status. Use filters to narrow results.', {
    ...
    server.tool('update_context_board', 'Write an agent entry to the shared context board. This is how agents communicate observations, proposals, decisions, and artifacts.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    // --- get_project_status ---
    server.tool('get_project_status', 'Get a high-level overview: project phase, agent statuses, plan summary, and statistics.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
    }, async ({ workspacePath }) => {
        const manager = new BoardManager(workspacePath);
        if (!manager.exists()) {
