...
// ─── Git helpers ───
function getGitRemote(workspacePath) {
    try {
...
// ─── Registration ───
export function registerSync(server) {
    // --- sync_push ---
    server.tool('sync_push', 'Push the local .weaver project intelligence to the Weaver Hub server so teammates on the same branch can pull it. Syncs context, index, plan, code maps, docs, team state, and annotations.', {
        ...
        if (!remote) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'No git remote found. Initialize a git repo with a remote first.' }) }] };
        }
        const manager = new BoardManager(workspacePath);
        if (!manager.exists()) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'No .weaver project found. Run init_project first.' }) }] };
        }
        ...
        if (files.length === 0) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'No .weaver files to push.' }) }] };
        }
        // Get project name
        ...
        try {
            ...
            if (!res.ok) {
                return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: data.error ?? 'Push failed' }) }] };
            }
            // Log the sync event
            manager.logEvent({
                level: 'info',
    ...
    // --- sync_pull ---
    server.tool('sync_pull', 'Pull the latest .weaver project intelligence from the Weaver Hub server. This downloads the shared context, index, plan, code maps, docs, team state, and annotations that teammates have pushed.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (!remote) {
            return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'No git remote found.' }) }] };
        }
        ...
        try {
            ...
            if (!res.ok || !data.exists) {
                return { content: [{ type: 'text', text: JSON.stringify({ success: false, message: data.message ?? `No snapshot found for branch "${branch}"` }) }] };
            }
            const weaverDir = path.join(workspacePath, '.weaver');
    ...
    // --- sync_status ---
    server.tool('sync_status', 'Check the sync status between your local .weaver and the Weaver Hub. Shows version, last push, available branches, and which teammates have contributed.', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        hubUrl: z.string().optional().describe(`Hub server URL (default: ${DEFAULT_HUB_URL})`),
    }, async ({ workspacePath, hubUrl }) => {
        const hub = hubUrl ?? DEFAULT_HUB_URL;
