...
function buildClassMap(index) {
    ...
    for (const file of index.files) {
        for (const cls of file.classes) {
            ...
            for (const m of cls.methods) {
                if (m.returnType)
                    extractTypeRefs(m.returnType, typeRefs);
            }
            for (const p of cls.properties) {
                if (p.type)
                    extractTypeRefs(p.type, typeRefs);
            }
    ...
    const seenIfaceIds = new Set();
    const uniqueInterfaces = interfaces.filter(i => {
        if (seenIfaceIds.has(i.id))
            return false;
...
function extractTypeRefs(typeStr, refs) {
    ...
    for (const [dir, files] of dirMap) {
        ...
        for (const file of files) {
            ...
            for (const fn of file.functions) {
                if (fn.exported)
                    publicAPI.push(fn.name);
            }
            for (const cls of file.classes) {
                if (cls.exported)
                    publicAPI.push(cls.name);
            }
        }
        modules.push({
            id: moduleId,
        ...
        for (const file of files) {
            for (const imp of file.imports) {
                ...
                const targetFile = index.files.find(f => f.path === targetPath ||
                ...
                if (targetDir === dir)
                    continue; // Skip intra-module imports
...
function detectLayers(modules, connections) {
    ...
    const entryModules = []; // High out-degree, low in-degree
    const coreModules = []; // Mix of both
    const utilModules = []; // High in-degree, low out-degree (shared/utility)
    const leafModules = []; // Low everything
    for (const mod of modules) {
    ...
    if (entryModules.length > 0)
        layers.push({ name: 'Entry Points', modules: entryModules });
    if (coreModules.length > 0)
        layers.push({ name: 'Core', modules: coreModules });
    if (utilModules.length > 0)
        layers.push({ name: 'Shared / Utilities', modules: utilModules });
    if (leafModules.length > 0)
        layers.push({ name: 'Standalone', modules: leafModules });
    return layers;
}
// ─── Call Graph Builder ───
function buildCallGraph(index) {
            ...
            for (const callsite of fn.callsites) {
                ...
                if (!targetId) {
                    ...
                    for (const imp of file.imports) {
                        if (imp.names.includes(callsite.name)) {
                            ...
                            const matchingFn = functions.find(f => f.name === callsite.name && f.exported);
                            if (matchingFn) {
                ...
                if (!targetId) {
                    const simpleName = callsite.name.includes('.')
                        ...
                        : callsite.name;
                    const match = functions.find(f => f.name === simpleName && f.id !== callerId);
                    if (match)
                        targetId = match.id;
                }
    ...
    const connectedFunctions = functions.filter(f => f.exported || f.calls.length > 0 || f.calledBy.length > 0);
    return { functions: connectedFunctions };
}
// ─── API Map Builder ───
function buildAPIMap(index) {
    const endpoints = [];
    for (const file of index.files) {
...
function extractPagesAPIRoutePath(filePath) {
...
export function registerCodeMaps(server) {
    server.tool('build_code_maps', 'Build traversable code maps from the project index: class hierarchy, module architecture, call graph, and API endpoints. Run after index_project and build_dependency_graph.', {
    ...
    server.tool('get_code_maps', 'Query code maps for specific views. Use during plan phase to understand codebase structure without reading every file. Views: summary (overview), classes (class hierarchy), modules (architecture), calls (function relationships), api (endpoints), file (single file context).', {
        workspacePath: z.string().describe('Absolute path to the workspace directory'),
        ...
        if (!fs.existsSync(codeMapsPath)) {
            return {
                content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'No code maps found. Run build_code_maps first.' }) }],
            };
        }
        const codeMaps = JSON.parse(fs.readFileSync(codeMapsPath, 'utf-8'));
        let result;
        switch (view) {
            case 'summary':
                result = {
            ...
            case 'file': {
                if (!file) {
                    return {
                        content: [{ type: 'text', text: JSON.stringify({ success: false, message: 'The "file" parameter is required for file view.' }) }],
                    };
                }
                ...
                const fileRelationships = codeMaps.classMap.relationships.filter(r => fileClassIds.has(r.from) || fileClassIds.has(r.to));
...
//# sourceMappingURL=code-maps.js.map