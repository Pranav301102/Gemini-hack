...
export function registerApproval(server: McpServer): void {
  ...
  server.tool(
    'request_plan_approval',
    'Set the plan status to "pending" so the user can review and approve it from the dashboard. Call this after the plan is finalized and ready for human review. The dashboard will show an approval gate UI.',
    {
    ...
    async ({ workspacePath, summary }) => {
      ...
      if (!board.plan) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ success: false, message: 'No plan exists yet. Create a plan first using the planning tools.' }) }],
        };
      }
  ...
  server.tool(
    'check_plan_approval',
    'Check the current approval status of the plan. Use this to poll after calling request_plan_approval. Returns the status (pending, approved, changes-requested) and any user comments.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    },
    async ({ workspacePath }) => {
