...
interface TechStackHint {
  file: string;
  tech: string;
  depsField?: string;
  detectors?: Record<string, string>; // dep name → framework label
}

const TECH_HINTS: TechStackHint[] = [
  {
...
function detectTechStack(rootPath: string): string[] {
...
function getProjectDescription(rootPath: string): string | undefined {
  // Try README.md
...
export function registerReadProject(server: McpServer): void {
  server.tool(
    'read_project',
    'Scan an existing codebase to auto-detect project structure, tech stack, and patterns. Initializes .weaver/ without asking questions. Use this for existing projects instead of init_project + gather_requirements.',
    {
    ...
    async ({ workspacePath }) => {
      ...
      const board = manager.initProject(
        projectName,
        description ?? `Existing ${techStack[0] ?? 'software'} project`,
        [], // No requirements — read mode skips questions
        techStack,
      );
