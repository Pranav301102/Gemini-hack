...
import type { ProcessInfo, AppLogLine } from '../types.js';
...
function inferLogLevel(line: string): 'info' | 'warn' | 'error' | 'debug' {
...
function processKey(name: string, workspacePath: string): string {
  return `${workspacePath}::${name}`;
}

export function registerAppRunner(server: McpServer): void {

  // --- launch_app ---
  server.tool(
    'launch_app',
    'Launch an application process (e.g., npm run dev) in the workspace and continuously monitor its stdout/stderr. Logs are parsed into WeaverEvents and written to .weaver/logs/. Errors are auto-promoted to context board entries.',
    {
  ...
  server.tool(
    'get_app_status',
    'Check the status of running app processes. Returns PID, uptime, status, and recent log lines.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    async ({ workspacePath, name: processName }) => {
      ...
      for (const [key, entry] of runningProcesses) {
        ...
        if (processName && entry.info.name !== processName) continue;
  ...
  server.tool(
    'stop_app',
    'Stop a running app process. Sends SIGTERM first, then SIGKILL after a timeout if the process does not exit.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'get_app_logs',
    'Get recent log output from a running (or recently stopped) app process. Supports level filtering.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
