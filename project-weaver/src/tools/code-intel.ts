...
import type { ProjectIndex, FileIndex, DependencyGraph, FunctionSignature, ClassDefinition, TypeDefinition } from '../types.js';

/**
 * Format a function into a compact readable signature.
 */
function formatFunction(fn: FunctionSignature): {
  name: string;
  signature: string;
  description: string;
  exported: boolean;
 ...
 */
function formatClass(cls: ClassDefinition) {
  return {
    name: cls.name,
 ...
 */
function formatType(t: TypeDefinition) {
  return {
    name: t.name,
    kind: t.kind,
 ...
 */
function getFileRelationships(filePath: string, graph: DependencyGraph | undefined, index: ProjectIndex) {
  const importsFrom: { file: string; names: string[]; description?: string }[] = [];
  const importedBy: { file: string; names: string[]; description?: string }[] = [];

  if (!graph) return { importsFrom, importedBy };
 ...
 */
function getRelatedFiles(filePath: string, graph: DependencyGraph | undefined): string[] {
  if (!graph) return [];
...
export function registerCodeIntel(server: McpServer): void {

  // ─── understand_file ───
  server.tool(
    'understand_file',
    'Get complete understanding of a file without reading source: enriched descriptions, all functions/classes/types, dependencies, dependents, and related files. This is agent memory — use it before reading any source file.',
    {
  ...
  server.tool(
    'search_codebase',
    'Search across the enriched code index by name or description. Find functions, classes, types, and variables without reading source files. Use this to discover existing code before writing new code.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
      ...
      for (const file of index.files) {
        ...
        if (kindFilter === 'function' || kindFilter === 'all') {
          for (const fn of file.functions) {
            if (exported && !fn.exported) continue;
        ...
        if (kindFilter === 'class' || kindFilter === 'all') {
          for (const cls of file.classes) {
            if (exported && !cls.exported) continue;
  ...
  server.tool(
    'get_dependency_graph',
    'Query the file dependency graph. Shows entry points, shared modules, directory clusters, circular dependencies. Use focus to zoom into a specific file\'s neighborhood.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    async ({ workspacePath, focus, depth, view }) => {
      ...
      if (!graph) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ success: false, message: 'No dependency graph found. Run build_dependency_graph first.' }) }],
        };
      }
...
function getNeighborhood(
  ...
  const queue: { file: string; depth: number; direction: 'imports' | 'importedBy' }[] = [];
  ...
  const neighbors: { file: string; depth: number; direction: string; description?: string }[] = [];
