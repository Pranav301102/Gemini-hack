...
import type {
  FileIndex, FunctionSignature, ClassDefinition,
  VariableDeclaration, TypeDefinition, ProjectIndex,
} from '../types.js';
...
function getAstLang(ext: string): Lang | null {
  switch (ext) {
    case '.ts': return Lang.TypeScript;
    case '.tsx': case '.jsx': return Lang.Tsx;
    case '.js': case '.mjs': case '.cjs': return Lang.JavaScript;
    default: return null;
  }
}
...
function findChild(node: SgNode, kind: string): SgNode | null {
  return node.children().find(c => c.kind() === kind) ?? null;
}

function findChildren(node: SgNode, kind: string): SgNode[] {
  return node.children().filter(c => c.kind() === kind);
}

function findNamedChild(node: SgNode, kind: string): SgNode | null {
  return node.children().find(c => c.isNamed() && c.kind() === kind) ?? null;
}

/** Extract JSDoc description from a comment node */
function extractJSDoc(commentText: string): string | undefined {
...
function getJSDocFromPrev(node: SgNode): string | undefined {
...
function getFileDescription(root: SgNode): string | undefined {
...
function extractParams(paramsNode: SgNode | null): { name: string; type?: string }[] {
  ...
  for (const child of paramsNode.children()) {
    ...
    if (kind === 'required_parameter' || kind === 'optional_parameter') {
      ...
      if (nameNode) results.push({ name: nameNode.text() });
...
function getReturnType(node: SgNode): string | undefined {
...
function hasJSXReturn(node: SgNode): boolean {
  ...
  if (!body) {
    ...
    return node.children().some(c =>
      c.kind() === 'jsx_element' || c.kind() === 'jsx_self_closing_element' || c.kind() === 'jsx_fragment'
    );
  }
...
function isReactFCType(typeText: string | undefined): boolean {
...
function extractWithAST(content: string, ext: string): ExtractResult {
  const lang = getAstLang(ext);
  if (!lang) throw new Error(`No AST lang for ${ext}`);
...
function processTopLevel(node: SgNode, isExported: boolean, result: ExtractResult): void {
...
function processExportStatement(node: SgNode, result: ExtractResult): void {
  ...
  if (exportClause) {
    for (const spec of findChildren(exportClause, 'export_specifier')) {
      ...
      if (exportedName) result.exports.push(exportedName);
    }
    return;
  }
...
function extractFunctionDecl(node: SgNode, exported: boolean): FunctionSignature {
...
function processVariableDeclaration(
  ...
  const varKind: 'const' | 'let' | 'var' = (kindMatch?.[1] as 'const' | 'let' | 'var') ?? 'const';
  ...
  for (const declarator of findChildren(node, 'variable_declarator')) {
    ...
    if (!nameNode) continue; // Skip destructured declarations for now
    ...
    if (arrowFn || funcExpr) {
      ...
      const valueNode = declarator.children().find(c =>
        c.isNamed() && c.kind() !== 'identifier' && c.kind() !== 'type_annotation'
      );
...
function extractInnerFunctions(body: SgNode, parentName: string, result: ExtractResult): void {
...
function extractClassDecl(node: SgNode, exported: boolean): ClassDefinition {
...
function extractInterfaceDecl(node: SgNode): TypeDefinition {
  const nameNode = findNamedChild(node, 'type_identifier');
...
function extractTypeAlias(node: SgNode): TypeDefinition {
  const nameNode = findNamedChild(node, 'type_identifier');
  const jsDoc = getJSDocFromPrev(node);

  // Try to extract union members for type aliases like `type X = 'a' | 'b' | 'c'`
  const valueNode = node.children().find(c =>
    c.isNamed() && c.kind() !== 'type_identifier' && c.kind() !== 'type_parameters'
  );

  let values: string[] | undefined;
  let fields: { name: string; type: string; optional?: boolean }[] | undefined;
...
function extractUnionMembers(node: SgNode): string[] {
...
function extractEnumDecl(node: SgNode): TypeDefinition {
  const nameNode = findNamedChild(node, 'identifier');
  ...
  if (body) {
    for (const member of body.children()) {
      if (member.kind() === 'enum_assignment') {
        ...
        if (propId) values.push(propId.text());
...
function extractImportStatement(node: SgNode, result: ExtractResult): void {
...
function extractPython(content: string): ExtractResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const getDocstring = (startIdx: number): string | undefined => {
    ...
    if (classMatch) {
      ...
      for (let j = i + 1; j < Math.min(i + 200, lines.length); j++) {
        ...
        if (memberLine.match(/^\S/) && j > i + 1) break;
...
function extractGo(content: string): ExtractResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ifaceMatch) {
      const td: TypeDefinition = { name: ifaceMatch[1], kind: 'interface', description: getComment() };
      ...
      if (fields.length > 0) td.fields = fields;
      types.push(td);
    }
...
function indexFile(filePath: string, rootPath: string): FileIndex | null {
...
function walkAndIndex(rootPath: string): FileIndex[] {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        if (INDEXABLE_EXTENSIONS[ext]) {
          ...
          if (fileIndex) results.push(fileIndex);
        }
      }
    }
  }
...
export function registerIndexer(server: McpServer): void {

  server.tool(
    'index_project',
    'Build a rich code index using AST parsing: extracts functions (with JSDoc, params, inner functions), classes, interfaces with fields, enums with values, variables, imports/exports. Writes to .weaver/index.json. Run after read_project.',
    {
    ...
    async ({ workspacePath }) => {
      if (!fs.existsSync(workspacePath)) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ success: false, message: `Path does not exist: ${workspacePath}` }) }],
        };
      }
      ...
      const fileTree = fileIndexes.map(f => ({ path: f.path, size: f.size, type: 'file' as const }));
          ...
          text: JSON.stringify({
            ...
            summary: {
              ...
              topFiles: fileIndexes
                .sort((a, b) =>
                  (b.functions.length + b.classes.length + b.types.length) -
  ...
  server.tool(
    'get_project_index',
    'Query the code index built by index_project. Search for functions, classes, interfaces, types, variables by file path, language, or name query. Returns rich data including JSDoc descriptions, interface fields, enum values.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    async ({ workspacePath, filePath, language, query, includeImports, includeVariables }) => {
      ...
      const output = results.map(f => {
        ...
        if (includeImports) base.imports = f.imports;
        if (includeVariables) base.variables = f.variables;
        return base;
      });
