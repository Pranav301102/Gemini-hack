...
export function registerPlanner(server: McpServer): void {

  // --- run_plan ---
  server.tool(
    'run_plan',
    'Start the planning phase. Assembles context from the project index and returns structured instructions for the Architect + PM brainstorm collaboration.',
    {
...
Follow Steps 1-4 from the existing project flow above, but base proposals on the gathered requirements instead of an index.`,
  ...
  server.tool(
    'add_proposed_change',
    'Add a proposed change to the plan. Groups changes into logical ChangeGroups. Creates a new group if the groupId does not exist.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'add_brainstorm_entry',
    'Record a brainstorm discussion entry between agents. Used to log observations, proposals, questions, agreements, and decisions during the planning phase.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'save_plan',
    'Finalize and save the project plan. Updates the plan with summary, goals, approach, risk assessment, file map, and diagrams. Sets the project phase to "ready".',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    async ({ workspacePath, summary, goals, approach, architectureNotes, riskAssessment, diagrams }) => {
      ...
      const fileChanges = new Map<string, { changeId: string; changeType: string; summary: string }[]>();
      ...
      const fileMap = Array.from(fileChanges.entries()).map(([file, changes]) => {
        ...
        const priorityOrder = { 'must-have': 0, 'should-have': 1, 'nice-to-have': 2 } as const;
        const maxPriority = priorities.sort((a, b) => (priorityOrder[a as keyof typeof priorityOrder] ?? 2) - (priorityOrder[b as keyof typeof priorityOrder] ?? 2))[0] ?? 'nice-to-have';
        ...
        return {
          ...
          changes: changes as { changeId: string; changeType: 'create' | 'modify' | 'refactor' | 'delete' | 'extend'; summary: string }[],
          totalChanges: changes.length,
          maxPriority: maxPriority as 'must-have' | 'should-have' | 'nice-to-have',
        };
      });
  ...
  server.tool(
    'get_plan',
    'Read the current project plan. Supports different views: summary, full, file-map, changes, discussion.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'get_file_change_map',
    'Get all proposed changes for a specific file. Shows what will change in that file and why.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    ...
    async ({ workspacePath, filePath }) => {
      ...
      const detailedChanges = plan.changeGroups.flatMap(g =>
        g.changes.filter(c => c.file === filePath).map(c => ({
          ...c,
          group: g.name,
          groupId: g.id,
        })),
