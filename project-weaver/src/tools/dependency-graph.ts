...
import type { DependencyEdge, DependencyGraph, ProjectIndex } from '../types.js';
 ...
 */
function computeDependencyGraph(index: ProjectIndex): DependencyGraph {
 ...
 */
function detectCycles(edges: DependencyEdge[], allFiles: Set<string>): string[][] {
...
export function registerDependencyGraph(server: McpServer): void {
  ...
  server.tool(
    ...
    'Compute a file dependency graph from import/export data in the code index. Detects entry points, shared modules, directory clusters, and circular dependencies. Run after index_project.',
    {
