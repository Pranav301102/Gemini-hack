...
export function registerDocs(server: McpServer): void {
  ...
  server.tool(
    'add_doc',
    'Add a document to the centralized .weaver/docs.json collection. All project documentation should go here instead of scattered README/doc files in the codebase.',
    {
    ...
    async ({ workspacePath, category, title, content, agent, tags }) => {
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            success: true,
            message: `Document "${title}" added to ${DOC_CATEGORY_LABELS[category as DocCategory]}`,
            docId: doc.id,
            category,
          }),
        }],
  ...
  server.tool(
    'get_docs',
    'Query the centralized documentation collection. Filter by category, tag, search text, or specific ID.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'update_doc',
    'Update an existing document in the collection. Records the previous version as a revision.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
  ...
  server.tool(
    'list_doc_categories',
    'List all documentation categories with document counts and last update timestamps.',
    {
      workspacePath: z.string().describe('Absolute path to the workspace directory'),
    },
    async ({ workspacePath }) => {
      ...
      const categoryMap = new Map<string, { count: number; latestUpdate: string }>();
      ...
      const categories = Array.from(categoryMap.entries()).map(([name, data]) => ({
        name,
        label: DOC_CATEGORY_LABELS[name as DocCategory],
        count: data.count,
